...
sudo mkdir -p ~/backup
sudo cp -i ~/.kube/config ~/backup/ >> /dev/null
sudo sed -e 's/6443/9663/g' -i ~/.kube/config
sudo cp -i /etc/kubernetes/manifests/kube-apiserver.yaml ~/backup/ >> /dev/null
...
sudo sleep 10
sudo echo " Opps ... Looks like your cluster is broken, Please fix it ... "
